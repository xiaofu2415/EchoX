...
| 千问模型 | 定位 | 适用场景 |
| --- | --- | --- |
| `qwen3.5-livetranslate-flash-realtime` | 当前首选 | 中英互译，支持音频+文本双语输出，适合世界杯、访谈和新闻直播 |
| `qwen3-livetranslate-flash-realtime` | 备选双语同传 | 中英互译，支持音频+文本双语输出 |
| `gummy-realtime-v1` | 历史模型 | 预计 9 月开始下架，不推荐新配置 |

WebSocket 实时流会持续向模型发送播放器音频，因此模型必须能够直接接收并理解音频。仅支持文本输入的模型无法独立完成这条实时链路，需要额外增加 STT 语音转写步骤。千问 Qwen3.5 / Qwen3 实时双语同传模型原生支持音频流和双语文本输出，更适合直播翻译，也是本插件的首选配置。
